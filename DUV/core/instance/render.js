    ...
    if (vnode.nodeType == 3) {//是文本节点
        ...
        if (templates) {
            ...
            for (let i = 0; i < templates.length; i++) {
                let templateValue = getTemplatesName([vm._data, vnode.env], templates[i])//当前节点的参数可以来自于Due对象,还可以来自于父级节点
        ...
        if (templates) {
            for (let i = 0; i < templates.length; i++) {
                let templateValue = getTemplatesName([vm._data, vm.env], templates[i])
                if (templateValue) {
...
function getTemplatesName(objs, templateName) {
